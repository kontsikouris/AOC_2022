...
isTreeVisible dim@(d1,d2) point@(n,m) gridMap 
    | n == 1 || n == d1 || m == 1 || m == d2 = 1
    | or (traverseGridT dim point $ checkVisibilityT point (fromJust $ Map.lookup point gridMap) gridMap) = 1
    | otherwise = 0
...
scenicScore dim@(d1,d2) point@(n,m) gridMap
    | n == 1 || n == d1 || m == 1 || m == d2 = 0
    | otherwise                              = product $ traverseGridT dim point $  scenicScoreT point (fromJust $ Map.lookup point gridMap) gridMap
...
traverseGridT :: Dim -> Point -> ([Point] -> a) -> [a]
traverseGridT d@(d1,d2) c@(n,m) f = map f $ coerce pointLL
    where   rowf (a,b) c = (c,b)
            colf (a,b) c = (a,c)
            l1  = coerce $ map (\f -> map (f c)) [rowf,rowf, colf, colf]             :: ZipList ([Int] -> [Point])
